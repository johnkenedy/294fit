...
import com.example.gym2.data.service.WorkoutTimerService.Companion.TIMER_RUNNING
        ...
        Dialog(onDismissRequest = { openDialog = false }) {
            Surface(
                ...
                Column(
                    ...
                    SubHeading(text = stringResource(id = R.string.edit_set))
                    ...
                    Row(
                        ...
                        Title(text = stringResource(id = R.string.reps).lowercase())
                        TextField(
                            value = reps,
    ...
    Surface(
        ...
        if (isWorkoutStarted) {
            BackHandler(true) {
                Toast.makeText(
                    context,
                    context.getString(R.string.workout_ongoing),
                    Toast.LENGTH_SHORT
                ).show()
            }
        }

        if (workoutState.exerciseItems?.size!! > 0) exerciseProgress =
            "${currentSetItemIndex + 1}/${workoutState.exerciseItems?.size}"
