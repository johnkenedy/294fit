package com.example.gym2.data.repository
...
class WorkoutRepositoryImpl @Inject constructor(
    ...
    override suspend fun deleteWorkoutPlan(workoutPlanId: String, uid: String) {
        withContext(Dispatchers.IO) {
            try {
                userCollection.document(uid).collection("workout_plan").document(workoutPlanId)
                    .delete()
                userCollection.document(uid).collection("workout_plan").document(workoutPlanId)
    ...
    override suspend fun getWorkoutPlan(uid: String): Resource<QuerySnapshot> {
        return withContext(Dispatchers.IO) {
            try {
                val result = userCollection.document(uid).collection("workout_plan").get().await()
    ...
    override suspend fun addNewExercise(exercise: Exercise, uid: String): Resource<Void> {
        return withContext(Dispatchers.IO) {
            try {
                val result = userCollection.document(uid).collection("exercises")
    ...
    override suspend fun getHistoryDataDetails(
        exerciseId: String,
        uid: String
    ): Resource<QuerySnapshot> {
        return withContext(Dispatchers.IO) {
            try {
                val result = userCollection.document(uid).collection("exercise_history")
